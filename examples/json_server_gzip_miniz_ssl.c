...
#define NS_OPENSSL_SUPPORT
#define NS_GZIP_SUPPORT
#include "nanocserver/nanojson.h"
#include "nanocserver/nanoserv.h"
...
int main(){
    Server* serv = ns_create_server(NULL, 3000, true, NS_SUPPORTED_COMPRESSION_GZIP);
    if(serv == NULL){
        fprintf(stderr, "Failed to initialize server\n");
        return 1;
    }

    bool s = ns_setup_ssl(serv, "cert/server.crt", "cert/server.key");
    // if(!s)
    //     abort();

    ns_serve_file(serv, "/example", "nanocserver/examples/json_with_server.c");
    ns_serve_file(serv, "/conv_ex.json", "./example.json");

    ns_serve_file(serv, "/mapping.json", "./site-mapping.json");
